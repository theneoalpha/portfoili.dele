...
router.post("/contact", async (req, res) => {
  ...
  const { name, email, phone,work,text} = req.body;

  if (!name || !email || !work || !text ) {
  ...
  try {
    ...
    const user = new User({ name, work, email, phone, text });
    ...
    } else res.status(500).json({ message: "Message Not sent" });
  } catch (err) {
    console.log(err);
  }
});
